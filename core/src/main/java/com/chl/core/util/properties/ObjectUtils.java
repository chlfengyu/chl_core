...
/**
 * @author Tera 只对POJO进行处理，目前主要实现了两个功能：1，POJO类的初始化。2，POJO类的toString。
 *         循环嵌套的问题使用图的遍历算法实现。
 */
public class ObjectUtils {
	...
	public static Map<String, Object> describe(Object obj) {
		...
		try {
			...
			throw new RuntimeException("Can't describe ["
	 ...
	 * 注意：除了"数组类型"说明中列出的"char[], int[], double[], String[], Date[]"
	 * 之外，其他的类型均视为自定义对象数组。
	 * 
	 * @param obj
	 *            obj
				...
				if (fieldType.equals(objectName) || isVisited(field)) {
					...
					if (fieldType.startsWith("[Z")) {
						...
						if (field.get(obj) == null || objectField.length == 0) {
							String className = fieldType.substring(2,
									fieldType.length() - 1);
	...
	/**
	 * 打印POJO对象的属性，包括简单类型：char, int, double, String, Object(自定义的) 数组类型：char[],
	 * int[], double[], String[], Date[], Object[](自定义的)。 List类型：List<String>,
	 * List<Date>, List<Object>
	 * 注意：除了"数组类型"说明中列出的"char[], int[], double[], String[], Date[]"
	 * 之外，其他的类型均视为自定义对象数组。
	 * 
	 * @param obj
	...
	private static void toString(Object obj) {
		...
		try {
			...
			for (int i = 0; i < fields.length; i++) {
				...
				// 基本属性
				if (fieldType.equals("boolean") || fieldType.equals("char")
					...
					buffer.append(field.getName() + ": ")
							.append(formatDate((Date) field.get(obj)))
							.append("\n");
				} else if (fieldType.startsWith("[")) {
					// 数组处理
					// bool 数组
					if (fieldType.startsWith("[Z")) {
						...
						// double 数组
						double[] doubleField = (double[]) field.get(obj);
						buffer.append("\ndouble[] " + field.getName()
								+ " Size:");
						if (field.get(obj) != null) {
							buffer.append(doubleField.length + "\n");
							for (int j = 0; j < doubleField.length; j++) {
								buffer.append(field.getName() + "[" + j + "]:")
										.append(doubleField[j]).append("\n");
							}
						} else {
							buffer.append(0).append("\n");
						}
	...
	private static void addVisited(String className) {
		if (className == null || className.equals("boolean")
				|| className.equals("char") || className.equals("int")
